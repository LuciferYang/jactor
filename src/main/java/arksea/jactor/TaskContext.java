...
public class TaskContext {
    ...
    private final Map<String, Actor> serviceMap = new HashMap();
    private final Map<String, Supervisor> supMap = new HashMap();
    private Timer timer = new Timer("TaskContext Timer", true);

    public static TaskContext instance() {
        return context;
    }

    public void start(String supervisorName, RestartStrategies restart, ChildInfo[] childs) {
        Supervisor sup = new Supervisor(supervisorName, restart, childs);
        supMap.put(supervisorName, sup);
    }

    public synchronized void stopAll() {
        for (Supervisor s : supMap.values()) {
            s.stopAll();
        }
        supMap.clear();
    }

    public void stop(String supervisorName) {
        Supervisor sup = supMap.remove(supervisorName);
        if (sup != null) {
            sup.stopAll();
        }
    }
    ...
    protected void put(String name, Actor svc) {
    ...
    protected Actor get(String name) {
        synchronized (serviceMap) {
    ...
    public Message call(String dest, Message callmsg, long timeout) {
        assert (!dest.equals(Thread.currentThread().getName()));
        if (timeout == 0) {
