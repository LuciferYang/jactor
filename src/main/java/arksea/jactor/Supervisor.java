...
public class Supervisor implements Thread.UncaughtExceptionHandler {
    ...
    private final List<ChildInfo> childList; //保留子任务启动顺序
    ...
    protected Supervisor(String name, RestartStrategies restart, ChildInfo[] childs) {
        synchronized(this) {
            ...
            childList = java.util.Collections.unmodifiableList(Arrays.asList(childs));
            startAll();
    ...
    private Thread createChild(ChildInfo info) {
        try {
            ChildTask child;
            if (Actor.class.isAssignableFrom(info.clazz)) {
                ...
                TaskContext.instance().put(info.name, (Actor) child);
            } else {
